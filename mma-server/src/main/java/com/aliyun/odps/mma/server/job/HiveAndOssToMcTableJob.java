...
public class HiveAndOssToMcTableJob extends AbstractTableJob {

  private static final Logger LOG = LogManager.getLogger(HiveAndOssToMcTableJob.class);

  HiveAndOssToMcTableJob(
      Job parentJob,
      JobRecord record,
  ...
  DirectedAcyclicGraph<Task, DefaultEdge> generateDag() throws Exception {
    ...
    try {
      OssConfig ossConfig = new OssConfig(
          config.get(AbstractConfiguration.DATA_SOURCE_OSS_ENDPOINT_INTERNAL),
          config.get(AbstractConfiguration.DATA_SOURCE_OSS_ENDPOINT_EXTERNAL),
          config.get(AbstractConfiguration.DATA_SOURCE_OSS_BUCKET),
          config.get(AbstractConfiguration.DATA_DEST_OSS_ROLE_ARN),
      ...
      String[] locations = OssUtils.getOssPaths(
          config.get(AbstractConfiguration.DATA_SOURCE_OSS_PATH),
      ...
      TableMetaModel hiveTableMetaModel = metaSource.getTableMeta(
          config.get(JobConfiguration.SOURCE_CATALOG_NAME),
          config.get(JobConfiguration.SOURCE_OBJECT_NAME));
