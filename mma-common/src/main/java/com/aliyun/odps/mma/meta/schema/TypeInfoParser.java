...
import com.aliyun.odps.mma.meta.schema.type.*;
...
public class TypeInfoParser {
  ...
  private TypeInfo parseTypeInfoInternal() {
    String token = peek();
    if ("NUMERIC".equals(token)) {
      return parseComplexNumericTypeInfo();
    } else if ("BIGNUMERIC".equals(token)) {
      return parseComplexBigDecimalTypeInfo();
//      return new ExtendedBigNumericType();
    } else if ("GEOGRAPHY".equals(token)) {
      return new ExtendedGeographyType();
    } else if ("INTERVAL".equals(token)) {
      return new ExtendedIntervalType();
    } else if ("TIME".equals(token)) {
      return new ExtendedTimeType();
    } else if ("RANGE".equals(token)) {
      return new ExtendedRangeType();
    }

    OdpsType typeCategory = OdpsType.valueOf(token);
  ...
  private TypeInfo parseComplexNumericTypeInfo() {
    int[] params = getDecimalParams();
    if (params == null)  {
      return new ExtendedNumericType();
    }
    return new ExtendedNumericType(params[0], params[1]);
  }

  // bigquery bignumeric<precision,scale>
  private TypeInfo parseComplexBigDecimalTypeInfo() {
    int[] params = getDecimalParams();
    if (params == null) {
      return new ExtendedBigNumericType();
    }

    return new ExtendedBigNumericType(params[0], params[1]);
//    return TypeInfoFactory.getDecimalTypeInfo(params[0], params[1]);
  }
  ...
  public static void main(String[] args) {
    TypeInfo typeInfo = getTypeInfoFromTypeString("BIGNUMERIC");
    System.out.println(typeInfo.getTypeName());

    TypeInfo typeInfo1 = getTypeInfoFromTypeString("ARRAY<BIGNUMERIC>");
    System.out.println(typeInfo1);
    TypeInfo typeInfo2 = getTypeInfoFromTypeString("STRUCT<x:BIGNUMERIC>");
